    ...
    <div class="container mx-auto">
        ...
        <form action="{{ route('clientes.update', $cliente) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input name="nombre" value="{{ old('nombre', $cliente->nombre) }}" class="input" required />
                @error('nombre') <p class="text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block">Email</label>
                <input name="email" type="email" value="{{ old('email', $cliente->email) }}" class="input" required />
                @error('email') <p class="text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block">Teléfono</label>
                <input name="telefono" value="{{ old('telefono', $cliente->telefono) }}" class="input" />
                @error('telefono') <p class="text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block">Código postal</label>
                <input name="codigo_postal" value="{{ old('codigo_postal', $cliente->codigo_postal) }}" class="input" />
