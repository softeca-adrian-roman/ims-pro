<x-app-layout>
    ...
    <div class="container mx-auto">
        ...
        <form action="{{ route('clientes.update', $cliente) }}" method="POST" class="space-y-4">
            ...
            <div>
                <flux:input name="telefono" label="Teléfono" value="{{ old('telefono', $cliente->telefono) }}" class="input" />
                @error('telefono') <p class="text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <flux:input name="codigo_postal" label="Código postal" value="{{ old('codigo_postal', $cliente->codigo_postal) }}" class="input" />
