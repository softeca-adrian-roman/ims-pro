    ...
    <div class="flex items-center justify-between mb-4">
        ...
        @can('crear clientes')
            <a href="{{ route('clientes.create') }}"
                class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md">Crear Cliente</a>
        @endcan
    </div>
    <div class="mb-4 bg-white p-4 rounded-md shadow">
        <div class="flex items-center justify-between mb-2">
            ...
            <div class="flex gap-2">
                <flux:button variant="filled" href="{{ route('clientes.export', request()->query()) }}"
                    icon="arrow-up-on-square-stack">
                    Exportar
                </flux:button>
                <form id="importForm" action="{{ route('clientes.import') }}" method="POST"
                    enctype="multipart/form-data" style="display: inline;">
                    @csrf
                    <input type="file" name="file" id="importFileInput" accept=".xlsx, .xls"
                        style="display: none;" onchange="this.form.submit()">
                    <flux:button variant="filled" icon="arrow-down-on-square-stack"
        ...
        <form method="GET" action="{{ route('clientes.index') }}"
            class="grid grid-cols-1 md:grid-cols-6 gap-2 items-end">
            <div>
            ...
            <div class="md:col-span-1 flex space-x-2">
                ...
                <flux:button variant="outline" href="{{ route('clientes.index') }}"
                    class="inline-flex items-center px-3 py-2 border rounded-md" icon="backspace"></flux:button>
            </div>
        </form>
    </div>
    @if (session('success'))
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-200">
                <tr>
                    ...
                    @can('ver vendedores')
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Vendedor
                        </th>
                    @endcan
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Provincia
                    </th>
                ...
                @foreach ($clientes as $cliente)
                    <tr class="{{ $loop->odd ? 'bg-gray-50' : 'bg-gray-100' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <flux:button size="sm" variant="outline" href="{{ route('clientes.show', $cliente) }}"
                                class="ml-2" icon="eye" title="Ver cliente"></flux:button>
                            @can('eliminar clientes')
                                <form action="{{ route('clientes.destroy', $cliente) }}" method="POST"
                                    style="display:inline-block" class="delete-form inline">
                                    @csrf
                                    @method('DELETE')
                                    <flux:button size="sm" variant="danger" type="submit" class="ml-2"
                                        icon="trash" title="Eliminar cliente"></flux:button>
                                </form>
                            @endcan
                            @can('editar clientes')
                                <flux:button size="sm" variant="filled" href="{{ route('clientes.edit', $cliente) }}"
                                    class="ml-2" icon="pencil" title="Editar cliente"></flux:button>
                            @endcan
                        </td>
                    </tr>
                @endforeach
